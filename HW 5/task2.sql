...
INSERT INTO restBill VALUES
    ...
    select avg(bill_total) as "Avg_Bill" from restBill where table_no = 2;

    select min(no_of_seats) as "Min", max(no_of_seats) as "Max", avg(no_of_seats) as "Avg" from restRest_table where room_name = 'Blue';

    select count(distinct table_no) as "Distinct_Tables" from restBill where waiter_no IN (004,002);
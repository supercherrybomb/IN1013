...
INSERT INTO restBill VALUES
    ...
    select cust_name from restBill where cust_name = '%Smith%' ORDER BY ASC cust_name;

    select distinct cust_name from restBill where cust_name = 'C%';

    select first_name from restStaff where headwaiter NOT NULL;
    
    select * from restBill where bill_date = '1602%';

    select distinct bill_date from restBill where bill_date = '15%' ORDER BY bill_date;
